...
sql <- glue(
  "
            SELECT 
              ...
              d_956490759 as completion_status,
              COUNT(d_956490759) AS count
            FROM  `nih-nci-dceg-connect-prod-6d04.FlatConnect.participants_JP`
            GROUP BY 
              site_name, d_956490759
            ORDER BY 
              site_name, d_956490759
            ")
...
```{r, echo = FALSE}
data_completion[c("site_name", "completion_status")] <- lapply(data_completion[c("site_name", "completion_status")], function(x) {
  if (is.character(x)) {
    ...
    x <- gsub("231311385", "Submitted", x)
    x <- gsub("615768760", "Started", x)
    x <- gsub("972455046", "Not Started", x)
  }
  x
})
...
```{r, echo = FALSE}
# Response by Site
completion_table <- data_completion %>%
  pivot_wider(names_from = completion_status, values_from = count, values_fill = 0)

completion_table = as.data.frame(completion_table)
completion_table = completion_table[2:11,c(1,3,4,5)]

# Add total row
completion_table <- completion_table %>%
  mutate(Total = Submitted + Started + `Not Started`)

# Calculate the total row and bind it to the bottom
completion_table <- bind_rows(
  ...
  completion_table %>%
    summarise(
      ...
      Started = sum(Started),
      `Not Started` = sum(`Not Started`),
      Total = sum(Total)
    )
)

# Calculate percentages for each cell and add them in parentheses
completion_table <- completion_table %>%
  mutate(
    Submitted = paste0(Submitted, " (", round((Submitted / Total) * 100, 1), "%)"),
    Started = paste0(Started, " (", round((Started / Total) * 100, 1), "%)"),
    `Not Started` = paste0(`Not Started`, " (", round((`Not Started` / Total) * 100, 1), "%)"),
    Total = paste0(Total, " (", round((Total / Total) * 100, 1), "%)")
  )

# Use kable to display the table with the new columns
kable_comp_by_site <- completion_table %>%
  kable("latex", booktabs = TRUE, 
        col.names = c("Site Name", "Submitted", "Started", "Not Started", "Total"), 
        align = c("l", "c", "c", "c", "c"),
        caption = "Completion Status by Site") %>%
  kable_styling(latex_options = "hold_position")
